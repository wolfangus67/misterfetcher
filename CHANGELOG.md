...
### Added
- Collapsible subsections in Configuration panel (Limits, Network Requests, Stream Source Cache Requests, Randomization, Caching & Logging)

### Changed
- Reduce toggle switch size for subsections to 60% of normal size for better visual proportion
- Remove outdated version variable from package init (version now tracked via git tags and CHANGELOG)

### Fixed
- Collapsible sections no longer flash open on page load before closing based on saved state
